...
if [ -n "$PRECOMMAND" ]; then
    echo "Running precommand: $PRECOMMAND"
    $PRECOMMAND &> /workspace/runpod-slim/comfyui.log
fi
...
if [ -n "$CUSTOM_ARGS" ]; then
    ...
    echo "Starting ComfyUI with environment CUSTOM_ARGS (overriding defaults): $CUSTOM_ARGS"
    nohup python main.py $CUSTOM_ARGS &> /workspace/runpod-slim/comfyui.log &
else
    if [ -s "$ARGS_FILE" ]; then
